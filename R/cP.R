...
#  suppressMessages(require(cubature))
  bef <- function(y){
    ...
    Int <- cubature::adaptIntegrate(g, lowerLimit = c(lambda, 0), upperLimit = c(Inf, lambda))
...
cpoisMME <- function(data){
  ...
  suppressWarnings(val.lambda <- rootSolve::uniroot(parfunc, lower =  0.1, upper = 50, extendInt = "yes")$root)
  return(val.lambda)
}
